...
solution solution_fold(const solution& s, const line_segment& fold) {
    ...
    solution ret;
    ret.source_positions = s.source_positions;
    ret.destination_positions = s.destination_positions;

    for (const auto& facet : s.facets) {
        std::vector<line_segment> segments;
        for (int i = 1; i < facet.size(); i += 1) {
            segments.push_back(line_segment(s.destination_positions[i-1], s.destination_positions[i]));
        }
        segments.push_back(line_segment(s.destination_positions[s.destination_positions.size()-1], s.destination_positions[0]));
        ...
        if (intersection_points.size() > 2) {
            ...
            ret.facets.push_back(facet);
            continue;
        }

        int i1 = ret.source_positions.size();
        int i2 = ret.source_positions.size() + 1;

        ret.source_positions.push_back(intersection_points[0].first);
        ret.source_positions.push_back(intersection_points[1].first);
        ret.destination_positions.push_back(intersection_points[0].first);
        ret.destination_positions.push_back(intersection_points[1].first);

        ret.facets.push_back(std::vector<int>{facet[0]});
        ret.facets.push_back(std::vector<int>{});
